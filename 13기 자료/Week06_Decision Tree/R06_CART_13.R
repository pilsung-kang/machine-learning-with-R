...
install.packages("party") # For CART
install.packages("ROCR") # For AUROC
...
for (i in 1:length(min_criterion))
{
  for ( j in 1:length(min_split))
  {
    for ( k in 1:length(max_depth))
    {
      ...
      cat("CART Min criterion:", min_criterion[i], ", Min split:", min_split[j], 
          ", Max depth:", max_depth[k], "\n")
