...
# Conduct LDA
lda_model <- lda(Target ~ ., data = trnData)

# Training result of LDA
plot(lda_model)

# Predict the unknown observations based on the LDA
lda_predict <- predict(lda_model, tstData)
names(lda_predict)
lda_predict$class
lda_predict$posterior
lda_predict$x
cfmatrix <- table(tstTargets, lda_predict$class)
cfmatrix

# Summarize the classification performances
Cperf = matrix(0,4,2)
Cperf[,1] <- perf_eval(cfmatrix)
...
Cperf[,2] <- perf_eval(tst_cm)
colnames(Cperf) <- c("LDA", "ANN")
Cperf
